    ...
    public boolean initialize(String cloud_server_address, String cloud_key, String cloud_secret) {
        ...
        cloud_recognizer.open(cloud_server_address, cloud_key, cloud_secret, new FunctorOfVoidFromCloudStatus() {
            ...
            public void invoke(int status, ArrayList<Target> targets) {
                ...
                synchronized (uids) {
                    boolean haswrrited = false;
                    for (Target t : targets) {
                        try {
                            if(!haswrrited && t.name() != null){
                                ...
                                for (int i = 0; i < names.length; i++) {
                                    if(i==0){
                                        TextView textView = (TextView) activity.findViewById(R.id.textView);
                                        textView.setText("名称："+names[i]);
                                    }else if(i==1){
                                        TextView textView = (TextView) activity.findViewById(R.id.textView2);
                                        textView.setText("颜色："+names[i]);
                                    }else if(i==2){
                                        TextView textView = (TextView) activity.findViewById(R.id.textView3);
                                        textView.setText("价格："+names[i]);
                                    }else if(i==3){
                                        TextView textView = (TextView) activity.findViewById(R.id.textView4);
                                        textView.setText("品牌："+names[i]);
                                    }
                                }
                            }
                        }catch (Exception e){
                            Log.e("moxingwang","设置名称异常",e);
                        }
